...
def api_init(app, table_list):
    ...
    for table in table_list:
        ...
        if not PermissionScope.query.filter_by(key=parent_key).first():
            db.session.add(
        ...
        for method in ["POST", "DELETE", "PATCH", "EXPORT", "IMPORT"]:
            key = table.get("key") + "_" + method.lower()
            if not PermissionScope.query.filter_by(key=key).first():
                db.session.add(
                ...
                db.session.add(PermissionScopeDetail(permission_key=permission_key, permission_scope_key=key))
