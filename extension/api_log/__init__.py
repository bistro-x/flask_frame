...
def init_app(app):
    ...
    def after_request(response):
        ...
        api_log.status = str(response.status_code)
...
if celery:
    ...
    def api_log_clean():
        ...
        api_log_retention_days = flask_app.config.get("API_LOG_RETENTION_DAYS", 30) # 日志保留时间
        db.session.execute(f"delete from z_know_info.api_log where create_time < (now() - interval '{api_log_retention_days} day');"  )
